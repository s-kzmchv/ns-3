...
for z in range(sizeSF + 1):
    ...
    for i in point:
        y_teorWithD.append(
            math.exp(-2 * _lambda * i * 0.12314) + 0.7 * 0.552631579 * _lambda * i * 2 * 0.12314 * math.exp(
                -2 * _lambda * i * 0.12314))
        # print(math.exp(-1 * _lambda * (i-1) * t_51ms[SF]))

    y_teorWhithOutD = []
    for i in point:
        y_teorWhithOutD.append(
            math.exp(-2 * _lambda * i * t_51ms[SF]) + 0.7 * 0.55555 * _lambda * i * 2 * t_51ms[SF] * math.exp(
                -2 * _lambda * i * t_51ms[SF]))
        # print(math.exp(-1 * _lambda * (i-1) * t_51ms[SF]))



    # y_teor4 = [0.9954, 0.7804, 0.7589, 0.6438, 0.5000, 0.4431]
    # y_teor4 = [0.9566111111111113, 0.8702477777777777, 0.7277955555555559, 0.6318922222222223, 0.5205122222222224, 0.49181888888888897]
    y_teor4 =  [0.9633333333333333, 0.88, 0.7834333333333333, 0.6903333333333333, 0.5924333333333334, 0.5423]
    y_loj = [0.9466666666666667, 0.8871333333333332, 0.7795, 0.6653333333333333, 0.5783333333333334, 0.5294666666666666]
    ...
    # print('значение рачет с защитой: ' + str(y_teorWithD))
    # print('значение рачет без защиты: ' + str(y_teorWhithOutD))

    # plt.plot(x, y_teor4, color="g", marker='o', label="Результы эксперементов")
    plt.plot(x, y_teor4, color="g", marker='o', label="Результы моделирования без имитации защиты")

    # plt.plot(x, averagePDR,  color="purple", marker='o',  label = "Результы моделирования")
    plt.plot(x, y_loj,  color="purple", marker='o',  label = "Результы моделирования  с имитацией защиты")
    ...
    # plt.plot(x, y_teorWithD,  color="b", marker='o', label = "Расчет по теор. формуле с учетом доп. защиты")
    # plt.plot(x, y_teorWhithOutD, color="r", marker='o', label="Расчет по теор. формуле")
