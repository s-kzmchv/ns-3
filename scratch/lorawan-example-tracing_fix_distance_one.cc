...
LoRaWANExampleTracing::SetupMobility ()
{
  ...
  {
    for (uint32_t i = 0; i <  m_nEndDevices; i++) // цикл по всем устройствам
    {
      
	  nodePositionList->Add (Vector (m_discRadius, 0, 0)); // установка end device с номером i на расстоянии m_discRadius по оси x от Getway
    }
   }
