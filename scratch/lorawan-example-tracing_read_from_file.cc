...
#include <iostream>
#include <iomanip>
#include <fstream> //библтотека для работы с потоковыми файлами 09.12.2017
                                ...
                                std::string phyTransmissionTraceCSVFileName, std::string phyStateTraceCSVFileName, std::string macPacketTraceCSVFileName, std::string macStateTraceCSVFileName, std::string edMsgTraceCSVFileName, std::string nsDSMsgTraceCSVFileName, std::string miscTraceCSVFileName, std::string nodesCSVFileName)
{
  ...
  std::cout << "My Starting simulation for " << m_totalTime << " s ...\n";
...
LoRaWANExampleTracing::SetupMobility ()
{
  ...
  std::ifstream f(File);//файл рассматривается как входной поток
  if (!(f.is_open())) // проверка имени файла имя которого задается в чтроке "char *File="node.txt""
    std:: cout<<"ERROR: not file "<<File;
  else
  {
    for (uint32_t i = 0; i <  m_nEndDevices; i++) // чтении из файла для заданного числа устройств
    {
      f>>x>>y>>r;
      std:: cout<<" x:"<<x<<" y:"<<y<<" r:"<<r<<std::endl;
      nodePositionList->Add (Vector (x, y, 0.0)); // установка координат for end device
    }
  }
  f.close();
  ...
  gwMobility.Install (m_endDeviceNodes);//делаем датчики неподвижными
  gwMobility.Install (m_gatewayNodes);// делаем БС неподвижными
}
...
LoRaWANExampleTracing::InstallApplications ()
{
  ...
  for (NodeContainer::Iterator i = m_endDeviceNodes.Begin (); i != m_endDeviceNodes.End (); ++i)
  {
    ...
    // Start the end device apps a  // gwMobility.Install (m_endDeviceNodes+m_gatewayNodes);t different times:
  ...
  for (ApplicationContainer::Iterator aci = endDeviceApp.Begin (); aci != endDeviceApp.End (); ++aci)
  {
    ...
    else if (m_drCalcMethodIndex == LORAWAN_DR_CALC_METHOD_RANDOM_INDEX)
    {
      f>>x>>y>>r;
      dataRateIndex =r;
      std:: cout<<" dataRateIndex ="<<(double)dataRateIndex <<std::endl;
    }
